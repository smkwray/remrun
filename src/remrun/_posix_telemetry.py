...
def _floor_nonnegative_mib(value: float) -> int:
    """Floor to whole MiB without turning an exhausted residual into capacity."""
    return max(0, int(math.floor(max(0.0, value) / MIB)) * MIB)
...
def _unprofiled_open_slot_allowance(
    ...
    *,
    ...
    return allowance, {**capacity, **details}, host_total
    ...
    with _locked_ledger(state_root) as ledger_path:
        ...
        if lease.get("allowance_basis") == "unprofiled_available_backed":
            ...
            if renewed_allowance < minimum or not sizing["safe"]:
                capacity["safe"] = False
            resized = renewed_allowance < int(lease["allowance_bytes"])
            lease = candidate
        ...
        return _admission_result(
            "admitted",
            "renewed_resized" if resized else "renewed",
            (
                "unprofiled reservation renewed with a lower live-backed allowance"
                if resized
                else "reservation renewed after atomically revalidating live target memory"
            ),
            policy=policy,
            capacity=capacity,
            lease=lease,
