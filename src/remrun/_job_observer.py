...
def _cleanup_stale_table_locked(
    ...
    for token, pid, identity, owner_kind, owner_key, witness_pid, witness_identity in rows:
        ...
        elif kind == "controller_operation":
            stale = _identity_state(
                int(witness_pid or pid), str(witness_identity or identity), snapshot
            ) in {"missing", "mismatch"}
...
def _operation_job_payload(record: dict[str, object]) -> dict[str, object]:
    """Report exact operation liveness without inventing process metrics.

    Controller operations such as Git sync span local Git, transport, and peer
    Git processes.  The initiating controller identity is a truthful liveness
    witness, but its own CPU/RAM is not a truthful aggregate for that operation.
    """
    return {
        "schema": QUERY_SCHEMA,
        "job_id": record["job_id"],
        ...
        "age_seconds": max(
            0.0, (time.time_ns() - int(record["started_at_ns"])) / 1e9
        ),
        "started_at_unix_ns": record["started_at_ns"],
        "root_process": {
            "pid": record["root_pid"],
        ...
        "metric_scope": "multi_endpoint_operation",
        "processes": {"current_count": None},
        "cpu": {
            "current_pct_one_logical_cpu": None,
            "normalized_host_pct": None,
        ...
        "threads": {"current_os_threads": None, "status": "unavailable"},
        "memory": {
            "current_bytes": None,
            "current_kind": "multi_endpoint_unavailable",
            "peak_bytes": None,
            "peak_status": "unavailable",
            "peak_detail": (
                "the operation spans controller, transport, and peer processes"
            ),
            "status": "unavailable",
        },
        "errors": [],
...
def _owned_sample(
    ...
    if kind == "controller_operation":
        state = _identity_state(
            int(record["witness_pid"]), str(record["witness_identity"]), snapshot
        )
        if state == "match":
            return "live", [], ""
        if state == "unknown":
            return "unknown", [], "the controller operation identity is inaccessible"
        return "stale", [], "the controller operation ended"
...
def _query(root: Path, sample_interval: float) -> dict[str, object]:
    ...
    jobs.extend(
        (
            _operation_job_payload(record)
            if record["owner_kind"] == "controller_operation"
            else _job_payload(record, first_rows, second_rows, elapsed, logical)
        )
