...
def finalize_durable_result(
    ...
    if isinstance(guard_token, str) and guard_token:
        ...
        try:
            reservation = MemoryReservation.from_payload({"lease": raw})
...
def _renewed_reservation_matches(
    ...
    if (
        ...
        or renewed.state_root != original.state_root
        or renewed.control_overhead_bytes != original.control_overhead_bytes
        or renewed.max_command_bytes != original.max_command_bytes
        ...
        return False
    if (
        renewed.allowance_bytes == original.allowance_bytes
...
class SSHPosixTransport(_SSHCommon):
    ...
    def _memory_guard_helper_path(self) -> str:
        return posixpath.join(
            ...
            "helpers",
            "remrun_posix_memory_guard_v3.py",
        )
