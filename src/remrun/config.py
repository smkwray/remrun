...
class RemrunConfig:
    ...
    # Strict declarative fleet task definitions. Workflow names and semantics
    # exist only under devices.toml [fleet.tasks]; core owns protocol primitives.
    fleet_tasks: dict[str, dict[str, Any]] = field(default_factory=dict)
...
def load_config(remrun_root: Path | None = None) -> RemrunConfig:
    ...
    fleet_doc = devices_doc.get("fleet", {})
    if not isinstance(fleet_doc, dict):
        from .fleet.config import FleetConfigError
        raise FleetConfigError("devices.toml [fleet] must be a table")
    unsupported_fleet = sorted(set(fleet_doc) - {"tasks"})
    if unsupported_fleet:
        from .fleet.config import FleetConfigError
        names = ", ".join(unsupported_fleet)
        raise FleetConfigError(
            f"devices.toml [fleet] contains unsupported general setting(s): {names}; "
            "put general fleet settings in config/defaults.toml"
        )
    tasks_doc = fleet_doc.get("tasks", {})
    if not isinstance(tasks_doc, dict):
        from .fleet.config import FleetConfigError
        raise FleetConfigError("devices.toml [fleet.tasks] must be a table")
    malformed_tasks = sorted(name for name, value in tasks_doc.items()
                             if not isinstance(value, dict))
    if malformed_tasks:
        from .fleet.config import FleetConfigError
        raise FleetConfigError(
            "devices.toml task definition(s) must be tables: " + ", ".join(malformed_tasks))
    ...
    fleet_tasks = {
        task: dict(spec)
        for task, spec in tasks_doc.items()
    }
