...
from .adapters import adapter_for, engine_for, memory_kind_for, option_bucket
from .models import (
    DeviceSnapshot, FleetTask, JobFeatures, PlacedBatch, PlacementResult,
)
from .profiles import profile_or_prior
...
def estimate_finish(indices: list[int], device: str, tasks: list[FleetTask],
    ...
    if not indices:
        return 0.0
    t0 = tasks[indices[0]]
    engine = engine_for(t0, device)
    bucket = option_bucket(t0)
    prof = profile_or_prior(profiles, t0.task_type, engine, device, bucket)
    fixed = float(prof.fixed_load_s or 0.0)
    var_rate = float(prof.var_per_unit_s or 0.0)
    variable = sum(var_rate * features[i].units(t0.task_type) for i in indices)
...
def predicted_resources(task: FleetTask, device: str, profiles: dict) -> tuple[float, float]:
    prof = profile_or_prior(profiles, task.task_type, engine_for(task, device),
                            device, option_bucket(task))
...
def fits(task: FleetTask, device: str, snap: DeviceSnapshot, profiles: dict,
         safety_fraction: float, fleet_cfg: dict | None = None) -> tuple[bool, str]:
    """Whether ``device`` can host ``task`` right now. Unknown probe values do not
    veto (degrade gracefully); only a *measured* shortfall skips a candidate."""
    ...
    if task.task_type != "cmd" and a is None:
        return False, "no adapter for this device"
    if a is not None:                       # model tasks: pool + capability gates
        pool = a.get("pool")
        if pool is not None:
        ...
        if snap.engines_available and eng not in snap.engines_available:
...
def _candidate_devices(task_type: str, snapshots: dict[str, DeviceSnapshot]) -> list[str]:
    """Devices worth considering for a task. ``cmd`` can run on any probed device;
    model tasks only on devices that have an adapter for them."""
    from .adapters import ADAPTERS
    if task_type == "cmd":
        return list(snapshots.keys())
    return [d for (t, d) in ADAPTERS if t == task_type and d in snapshots]
...
def _fitting_devices(task: FleetTask, snapshots: dict[str, DeviceSnapshot],
    ...
    fitting, skipped = [], {}
    for dev in _candidate_devices(task.task_type, snapshots):
...
def _greedy_split(indices: list[int], devices: list[str], tasks, features,
    ...
    order = sorted(indices, key=lambda i: features[i].units(tasks[i].task_type), reverse=True)
    for i in order:
...
def assign_group(indices: list[int], tasks: list[FleetTask], features: list[JobFeatures],
    ...
    """Place one group of jobs that share (task_type, option_bucket) and so can batch onto a
...
def _split_hysteresis(indices: list[int], t0: FleetTask, features: list[JobFeatures],
    ...
    better finish, and — for OCR — the page-count uncertainty (an approximate page count makes
    per-page time uncertain, and that error can dwarf a tiny split win near the ~74-page
    crossover). Replaces the fixed ``hysteresis_s=1.0`` (far too small vs a 100 s+ cold load)."""
    ...
    if t0.task_type == "ocr":
        approx_pages = sum(features[i].pages for i in indices
                           if getattr(features[i], "pages_approx", False))
        if approx_pages > 0:
            unc = float(fleet_cfg.get("page_uncertainty_frac", 0.25))
            var_rate = max((float(profile_or_prior(profiles, t0.task_type, engine_for(t0, d),
                                                   d, option_bucket(t0)).var_per_unit_s or 0.0)
                            for d in fitting), default=0.0)
            margin = max(margin, var_rate * approx_pages * unc)
    return margin
...
def plan_jobs(tasks: list[FleetTask], features: list[JobFeatures],
    ...
    """Group jobs by (task_type, option_bucket) and place each group. Makespan is
    ...
    for i, t in enumerate(tasks):
        groups.setdefault((t.task_type, option_bucket(t)), []).append(i)
