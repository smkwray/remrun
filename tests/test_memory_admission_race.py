...
def _reserve_unprofiled_worker(state_root: str, barrier, queue) -> None:
    from remrun import _posix_telemetry as telemetry

    telemetry._host_memory = lambda: (64 * GIB, 26 * GIB)
    telemetry._processes = lambda: {}
    telemetry._control_overhead_budget_bytes = lambda: 128 * MIB
    request = {
        "schema": 2,
        "op": "reserve",
        "state_root": state_root,
    ...
    queue.put(
        (
            result["status"],
            result["reason"],
            lease.get("allowance_bytes") if isinstance(lease, dict) else None,
        )
    )
...
def test_cross_controller_unknowns_receive_complementary_open_slot_shares(
    ...
    processes = [
        context.Process(
            target=_reserve_unprofiled_worker,
            args=(str(tmp_path / "unknown"), barrier, queue),
        )
        for _ in range(2)
    ]
    for process in processes:
