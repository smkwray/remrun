...
def test_render_uses_reported_target_for_controller_operation_and_keeps_metrics_blank():
    operation = job_row(
        target="CONTROLLER",
        reported_target="PEER",
        command={"label": "git-sync:push", "sha256": "b" * 64},
        cpu={"current_pct_one_logical_cpu": None, "normalized_host_pct": None},
        threads={"current_os_threads": None},
        memory={"current_bytes": None, "peak_bytes": None},
    )

    text = render_table([jobs.TargetJobsView("CONTROLLER", True, "ok", jobs=[operation])])

    assert "PEER" in text
    assert "git-sync:push" in text
    assert "  -" in text
...
def test_executor_registers_one_observation_for_a_batch(tmp_path, monkeypatch):
    ...
    from remrun.fleet.prepared import (
        RAW_COMMAND_SPEC, RAW_COMMAND_SPEC_ID, as_fleet_task, prepare_raw_command,
    )
    ...
    task = as_fleet_task(
        prepare_raw_command(["python", "-c", "pass"], device="LOCAL_SIM"),
        {**RAW_COMMAND_SPEC, "spec_id": RAW_COMMAND_SPEC_ID},
    )
    result = executor.run_batch(
        "LOCAL_SIM", [task], cfg, state_root=tmp_path / "state",
        job_ids=["a"], observation_id="batch-7",
    )
    assert result["ok"] is True
    assert len(captured) == 1
    assert captured[0].job_id == "batch-7"
    assert captured[0].project == "@fleet"
    assert captured[0].member_count == 1
